...
Backend:  http://192.168.1.115:8080
```
...
Với IP máy tính hiện tại là `192.168.1.115`:

```env
APP_BASE_URL=http://192.168.1.115:8080
APP_FRONTEND_URL=http://192.168.1.115:3000
...
```text
http://192.168.1.115:3000/reset-password?token=test
```
...
1. Từ Chrome/Safari điện thoại mở
   `http://192.168.1.115:8080/api/v1/auth/health` và nhận HTTP 200.
2. Mở `http://192.168.1.115:3000/reset-password?token=test` và thấy UI FAMS.
...
7. Đổi email:
   - nếu chưa sửa source: link API dùng host `192.168.1.115:8080`;
