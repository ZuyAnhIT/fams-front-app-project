...
- Sau khi đổi IP LAN, export lần đầu còn dùng Metro cache chứa IP cũ
  `192.168.1.11`. Đã export lại với `--clear` và kiểm tra trực tiếp bundle Android
  lẫn iOS đều chứa đúng `192.168.1.11:8080`.
- `GET /api/v1/auth/health` qua IP LAN hiện tại `192.168.1.11:8080`: HTTP
